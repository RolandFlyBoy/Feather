...
> This is part 5 of a 6-part series building a production Kanban app.
...
App Type
  ...
  Select type (simple, single-tenant, multi-tenant) [simple]: multi-tenant

Database
  Database name (PostgreSQL required) [kanban-saas]: kanban_saas
...
Admin Setup
  Platform admin email: admin@yourcompany.com
```

Multi-tenant apps require PostgreSQL, so there's no database *type* question —
only a name. **Type `kanban_saas`**: the default is the project name,
`kanban-saas`, and a hyphen in a Postgres database name has to be quoted
everywhere forever.

Cloud storage already defaults to `Y`; keep it, because Step 6 attaches files to
cards. Answer `n` to Redis caching — it defaults to `Y` and would make the app
expect a Redis server that nothing in this tutorial uses. Background jobs stay
at the default; the thread backend runs them in-process. The platform admin email is the
address `seeds.py` grants `is_platform_admin`, so use one you can sign in with.
...
`Account` and `AccountUser` both live in `models/account.py` — there is no
`models/account_user.py`. Keep them, along with `Log`: the scaffolded
`services/admin_service.py` and `seeds.py` import all three, and the app won't
start without them.
...
`tests/test_home.py` tests the route you just deleted, so it goes too —
otherwise `feather test` fails on a 404.
...
It runs on your laptop. [Tutorial 6: Deploying](06-deploying.md) puts it on a
€7/month server at your own domain, with automatic HTTPS, nightly backups and a
deploy that runs on every merge to `main`.
