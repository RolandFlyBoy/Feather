---
title: "File storage"
description: "One interface for local files and Google Cloud Storage, so moving between them is a config change."
icon: "hard-drive"
---

Files go to the local filesystem or to Google Cloud Storage, behind the same interface.
Switching between them is a change in `.env` rather than a change in your code.
