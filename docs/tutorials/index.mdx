...
That's not a shortcut around the real work. It's the point of the framework. Feather fixes
where files go, how services are shaped, and what components look like, then ships those
conventions in a `CLAUDE.md` that every project carries. Predictable structure is what
makes a described feature turn into working code, and `feather check` is what tells you
whether it did.

So you won't be typing out models here. You'll be reading what came back, running the
checks, and learning the framework from the shape of the result.
...
<Steps>
  <Step title="Scaffold" icon="folder-plus">
    ...
    matter and why. This is the one step worth doing by hand, since the answers decide what
    gets generated.
  </Step>
