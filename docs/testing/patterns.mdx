...
<Tip>
  Because routes stay thin and services hold the logic, most assertions belong in service
  tests. Route tests then only prove the wiring: the right decorator, the right status
  code, the right key in the response.
</Tip>
