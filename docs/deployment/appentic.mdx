...
<Steps>
  ...
  <Step title="Keep the Feather setup" icon="zap">
    ...
    `/health/ready`. The project also gets `SIGN_IN_RELAY_URL` and `SIGN_IN_RELAY_TOKEN`,
    so an app with [email sign-in](/features/authentication#setup-email) can send its
    sign-in links with nothing to set up.
...
OAuth client. Email sign-in links work on the new domain straight away.
