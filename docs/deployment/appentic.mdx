...
- Your migrations are committed. `feather new` leaves `migrations/versions` empty, so
  generate the first one locally and commit it, or the release command has nothing to
  apply and your app starts against an empty database.
- You have an Appentic account with credits.

Run this locally first:

```bash
feather db migrate -m "Initial migration"   # if migrations/versions is empty
feather check
