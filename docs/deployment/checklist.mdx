...
<Check>
  **Your migrations are in the repository.** `migrations/versions` holds the first
  migration and every one since, and they apply from an empty database. The
  `migrations` check in `feather security-check` covers the first part.
</Check>
